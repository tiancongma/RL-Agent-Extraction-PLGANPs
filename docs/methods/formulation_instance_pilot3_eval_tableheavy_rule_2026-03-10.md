...
- `10.1002/jps.24101`: GT=26 formulation rows, pred=26; over-seg=no, under-seg=no, boundary=mixed, non-form suppression=no, inheritance separation=no.
- `10.1016/j.ejpb.2004.09.002`: GT=22 formulation rows, pred=24; over-seg=yes, under-seg=no, boundary=broken, non-form suppression=not_applicable, inheritance separation=yes.
...
## Engineering readout
- Predicted formulation rows: 59 vs GT 57.
- Predicted candidate_non_formulation rows: 0 vs GT non-formulation rows 3.
- Compressed enum design appears workable: **mixed**.
- Next bottleneck: non-synthesis variant suppression and post-processing/test-condition classification.