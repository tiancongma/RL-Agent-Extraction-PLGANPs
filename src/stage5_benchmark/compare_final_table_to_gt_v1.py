...
def build_summary_markdown(
    *,
    ...
    lines = [
        ...
        f"- legacy_benchmark_valid_flag: `{'yes' if benchmark_valid else 'no'}`",
        "",
        "## GT Diagnostic Statement",
        "",
        "- This comparison is a full-pipeline diagnostic check over a manually reviewed GT subset.",
        "- Its purpose is to verify that a repair has propagated through the maintained mainline into the completed Stage 5 final formulation table.",
        "- The comparison object is the completed Stage 5 final formulation table, not an intermediate artifact.",
        "- No intermediate Stage2, Stage3, or reviewer-surface artifact is used as the system result.",
        "",
        "## Supported diagnostic views",
        "",
    ...
    lines.extend(
        [
            ...
            "- The authoritative fixed DEV15 skeleton workbook does not expose structured EE ground-truth fields, so no EE subset diagnostic comparison is emitted in this first full-pipeline run.",
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "",
            "## 6. GT Diagnostic Status",
            "",
            "- compare_mode: `diagnostic`",
            "- gt_subset_role: `mainline_integration_diagnostic`",
            "- benchmark_certification: `not_requested`",
            f"- legacy_benchmark_valid_flag: `{'yes' if benchmark_valid else 'no'}`",
            "- diagnostic_status: `full_pipeline_gt_diagnostic`",
            "- Reason: DEV15 GT comparison is used to validate mainline repair propagation and locate remaining extraction errors.",
            "",
...
def compare_final_table_to_gt(
    ...
    result = {
        ...
        "gt_subset_role": "mainline_integration_diagnostic",
        "benchmark_certification": "not_requested",
        "legacy_benchmark_valid_flag": benchmark_valid,
    }
...
def main(argv: list[str] | None = None) -> None:
    ...
    allow_explicit_diagnostic_final_table = args.final_table_tsv is not None
