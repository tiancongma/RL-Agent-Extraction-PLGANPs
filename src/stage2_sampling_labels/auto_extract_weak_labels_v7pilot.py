...
FEW_SHOT = r"""
Example JSON (style only):
{
  "schema_version": "weak_labels_v7",
  "paper_notes": "Two formulations are compared; PLGA MW is global in methods.",
  "formulations": [
    {
      "formulation_id": "F1",
      ...
      "fields": {
        "plga_mw_kDa": {
          "value": 50,
          "value_text": "PLGA 50 kDa",
          "scope": "global_shared",
          "membership_confidence": "medium",
          "evidence_region_type": "methods_sentence"
        },
        "drug_feed_amount_text": {
          "value": "10 mg",
          "value_text": "10 mg",
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Pilot weak_labels_v7 extractor (3-paper pilot oriented).")
    p.add_argument(
        "--manifest-tsv",
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    validate_models_or_raise([args.model], context="v7pilot preflight")
    ...
    default_out = Path(f"data/results/run_{ts}_v7pilot3_dev/weak_labels_v7pilot")
    out_dir = Path(args.out_dir) if args.out_dir else default_out
    out_dir.mkdir(parents=True, exist_ok=True)
    out_jsonl = out_dir / "weak_labels__v7pilot.jsonl"
    out_tsv = out_dir / "weak_labels__v7pilot.tsv"
