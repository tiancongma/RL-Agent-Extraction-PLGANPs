...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Pilot weak_labels_v7 extractor r3 (polymer-code + baseline-additive refinement).")
    p.add_argument(
        "--manifest-tsv",
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    validate_models_or_raise([args.model], context="v7pilot_r3 preflight")
    ...
    default_out = Path(f"data/results/run_{ts}_v7pilot3r3_dev/weak_labels_v7pilot_r3")
    out_dir = Path(args.out_dir) if args.out_dir else default_out
    out_dir.mkdir(parents=True, exist_ok=True)
    out_jsonl = out_dir / "weak_labels__v7pilot_r3.jsonl"
    out_tsv = out_dir / "weak_labels__v7pilot_r3.tsv"
