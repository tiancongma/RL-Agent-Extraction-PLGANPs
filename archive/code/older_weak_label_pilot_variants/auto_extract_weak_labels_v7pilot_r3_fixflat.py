...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Pilot weak_labels_v7 extractor r3_fixflat (flattening bugfix only).")
    p.add_argument(
        "--manifest-tsv",
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    validate_models_or_raise([args.model], context="v7pilot_r3_fixflat preflight")
    ...
    default_out = Path(f"data/results/run_{ts}_v7pilot3r3fixflat_dev/weak_labels_v7pilot_r3_fixflat")
    out_dir = Path(args.out_dir) if args.out_dir else default_out
    out_dir.mkdir(parents=True, exist_ok=True)
    out_jsonl = out_dir / "weak_labels__v7pilot_r3_fixflat.jsonl"
    out_tsv = out_dir / "weak_labels__v7pilot_r3_fixflat.tsv"
