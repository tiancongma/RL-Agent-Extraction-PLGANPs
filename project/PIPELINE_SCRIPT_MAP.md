...
| Stage 1 | Manifest, clean text, and tables | `src/stage1_cleaning/run_stage1_marker_pdf_extraction_v1.py` | `SUPPORTING_ENTRYPOINT` | Build and freeze the additive Marker PDF extraction surface for PDF records. It uses local `.venv_marker/bin/marker_single`, reuses frozen outputs when source/config hashes match, and writes governed Marker raw JSON, metadata, clean-text, and structure sidecars for later current+Marker fusion without rerunning Marker in downstream diagnostics. | `data/cleaned/index/manifest_current.tsv`; optional explicit scope keys | `data/cleaned/content/marker_pdf/`; `data/cleaned/index/key2marker_pdf_v1.tsv`; `data/cleaned/index/stage1_marker_pdf_extraction_summary_v1.tsv` |
| Stage 1 | Manifest, clean text, and tables | `src/stage1_cleaning/build_stage1_pdf_current_marker_fusion_v1.py` | `SUPPORTING_ENTRYPOINT` | Build additive current+Marker PDF fusion artifacts from existing current PDF text/structure plus frozen Marker PDF outputs. It preserves current clean text as compatibility authority, attaches Marker structure/text additively, and writes governed fused clean-text/structure indexes for later Stage1-controlled consumption. | `data/cleaned/index/key2txt.tsv`; `data/cleaned/index/key2structure.tsv`; `data/cleaned/index/key2marker_pdf_v1.tsv`; optional explicit scope keys | `data/cleaned/content/pdf_current_marker_fusion/`; `data/cleaned/index/key2stage1_pdf_fusion_v1.tsv`; `data/cleaned/index/stage1_pdf_current_marker_fusion_summary_v1.tsv` |
...
- Stage 5 remains one coarse namespace, but its internal value-layer design is
  now split into:
...
| `src/stage1_cleaning/run_stage1_marker_pdf_extraction_v1.py` | `STABLE_TOOL` | Governed Marker PDF extraction/freeze helper. It invokes the local Marker runtime (`.venv_marker/bin/marker_single`), writes frozen Marker raw JSON/meta/clean-text/structure artifacts under `data/cleaned/content/marker_pdf/`, promotes `key2marker_pdf_v1.tsv`, and reuses outputs unless source/config hashes change. Use this surface before any current+Marker clean-text/structure fusion; do not rerun Marker from downstream diagnostics. |
| `src/stage1_cleaning/build_stage1_pdf_current_marker_fusion_v1.py` | `STABLE_TOOL` | Governed current+Marker PDF fusion helper. It consumes current `key2txt.tsv`/`key2structure.tsv` plus frozen `key2marker_pdf_v1.tsv`, writes additive fused clean-text/structure artifacts, and does not run downstream Stage2/Stage5. |
| `src/stage1_cleaning/build_stage1_unified_current_marker_v1.py` | `STABLE_TOOL` | Governed unified Stage1 document-surface helper for PDF and HTML. It consumes current clean text/structure, frozen Marker PDF outputs when already available, and current Stage1 table-cell sidecar bindings, then writes a single Stage2-consumable text/structure/sidecar manifest surface without rerunning Marker/parsers or running downstream Stage2/Stage5. |
...
| `src/stage2_sampling_labels/denoise_stage2_source_text_s2_1b_v1.py` | `PLANNED_STABLE_TOOL` | Planned deterministic S2-1b source-denoise projection helper. It will delete only high-confidence source boilerplate/noise before S2-2 evidence construction, preserve raw source audit paths, write denoised text plus audit summaries, and must not perform formulation semantic discovery or table-importance selection. Register as an implemented stable tool only after the script exists and tests pass. |
...
| `src/stage2_sampling_labels/build_formulation_universe_discovery_v1.py` | `STABLE_TOOL` | Supporting diagnostic Stage2 formulation-universe discovery gate. It consumes explicit scope manifests plus current manifest enrichment, builds per-paper full-text/table prompt packages, optionally calls DeepSeek or Gemini/Gemma with JSON output, and emits one frozen formulation-universe TSV, excluded-candidate ledger, unresolved-candidate review queue, prompt/raw-response/parsed JSON artifacts, count-level DEV15 diagnostics, and `RUN_CONTEXT.md`. It is diagnostic/supporting only unless separately promoted through the maintained Stage2 semantic contract; it is not a completed Stage2 artifact, not a lawful Stage3 input, and not a Stage5 final table. |
| `src/stage2_sampling_labels/build_formulation_universe_adjudication_v1.py` | `STABLE_TOOL` | Supporting diagnostic adjudication surface for comparing formulation-universe discovery outputs across models. It consumes explicit discovery run directories, emits model count comparisons, disagreement ledgers, a provisional review-only adjudicated universe, and `RUN_CONTEXT.md`. GT counts are post-hoc diagnostics only; this output is not a completed Stage2 artifact, not a lawful Stage3 input, and not a Stage5 final table. |
| `src/stage2_sampling_labels/build_formulation_universe_risk_flags_v1.py` | `STABLE_TOOL` | Supporting GT-free risk flagger for formulation-universe discovery outputs. It consumes explicit discovery run directories and prompt/source signals, marks row-universe underexpansion, model disagreement, control/assay over-inclusion, large exclusion ledgers, and unresolved candidates, and may append observational row counts from explicit GPT35 or other Stage2/Stage5 lineage paths. It does not consume GT and does not create rows or values. |
| `src/stage2_sampling_labels/build_formulation_universe_row_audit_v1.py` | `STABLE_TOOL` | Supporting row-level evidence reviewer for formulation-universe outputs. It consumes an existing discovery run plus explicit paper-level risk flags, gives a reviewer LLM one proposed row and its evidence/context, and emits diagnostic row-level verdicts. It does not create rows, extract values, mutate the formulation universe, or produce benchmark-valid Stage2/Stage3/Stage5 artifacts. |
...
| `src/stage5_benchmark/build_s5_5_derived_values_v1.py` | `STABLE_TOOL` | S5-5 derived and unit-normalization sidecar helper. It consumes explicit accepted-direct values, currently registers `%w/v × mL -> mg` and `mg/mL × mL -> mg` derivations, concentration-mass-volume triangle solving, explicit role-ordered binary/ternary ratio mass solving, and narrow mass/volume/concentration unit-normalization projections. It writes derived/unit-normalization/provenance/review/summary/RUN_CONTEXT sidecars and marks derived or normalized rows ineligible for direct compare. |
